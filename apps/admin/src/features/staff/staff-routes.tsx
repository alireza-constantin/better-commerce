...
function StaffContent() {
  ...
  return <section className="mx-auto max-w-6xl space-y-6" dir="rtl">
    <header className="flex flex-wrap items-start justify-between gap-4">
      ...
      {canCreate && canAssignRoles ? <CreateStaffDialog disabled={pending} roles={assignableRoles} canAssignOwner={canAssignOwner} onSubmit={async (email, roleKeys) => { const result = await create.mutateAsync({ email, roleKeys }); await refresh(result); }} /> : null}
    </header>
...
function StaffCard(props: { readonly member: StaffProfile; readonly roles: readonly StaffRole[]; readonly disabled: boolean; readonly canAssignRoles: boolean; readonly canAssignOwner: boolean; readonly canSuspend: boolean; readonly onRoles: (keys: readonly string[]) => Promise<void>; readonly onStatus: () => Promise<void> }) {
  ...
  return <Card><CardHeader className="flex-row items-start justify-between gap-4"><div className="min-w-0"><CardTitle className="truncate text-base"><bdi dir="ltr">{member.email}</bdi></CardTitle></div><StatusBadge tone={member.status === 'active' ? 'success' : 'warning'}>{member.status === 'active' ? 'فعال' : 'معلق'}</StatusBadge></CardHeader><CardContent className="space-y-5"><div><p className="mb-2 text-xs font-medium text-muted-foreground">نقش‌های فعلی</p><div className="flex flex-wrap gap-2">{member.roles.length ? member.roles.map((role) => <StatusBadge key={role}>{roleLabel(role)}</StatusBadge>) : <span className="text-sm text-muted-foreground">بدون نقش</span>}</div></div><div className="flex flex-wrap gap-2">{canAssignRoles && !protectedOwner ? <Button disabled={disabled} onClick={() => setEditing(true)} size="sm" variant="outline">ویرایش نقش‌ها</Button> : null}{canSuspend && !protectedOwner ? <Button disabled={disabled} onClick={() => setConfirmStatus(true)} size="sm" variant="ghost" className="text-destructive">{member.status === 'active' ? 'تعلیق دسترسی' : 'فعال‌سازی دسترسی'}</Button> : null}</div></CardContent>
...
function CreateStaffDialog({ disabled, roles, canAssignOwner, onSubmit }: { readonly disabled: boolean; readonly roles: readonly StaffRole[]; readonly canAssignOwner: boolean; readonly onSubmit: (userId: string, roleKeys: readonly string[]) => Promise<void> }) {
  const [open, setOpen] = useState(false); const [email, setEmail] = useState(''); const [selected, setSelected] = useState<readonly string[]>([]); const valid = isEmail(email) && selected.length > 0;
  return <><Button onClick={() => setOpen(true)}><Plus aria-hidden="true" /> افزودن کارمند</Button><Dialog open={open} onOpenChange={setOpen}><DialogContent dir="rtl"><DialogHeader><DialogTitle>افزودن کارمند</DialogTitle><DialogDescription>ایمیل کاربری که در فروشگاه حساب دارد وارد کنید و نقش‌های اولیه را انتخاب کنید.</DialogDescription></DialogHeader><form className="space-y-5" onSubmit={(event) => { event.preventDefault(); if (!valid) return; void onSubmit(email.trim(), selected).then(() => { setOpen(false); setEmail(''); setSelected([]); }); }}><Field data-invalid={email.length > 0 && !isEmail(email)}><FieldLabel htmlFor="staff-email">ایمیل کاربر</FieldLabel><Input id="staff-email" dir="ltr" inputMode="email" onChange={(event) => setEmail(event.target.value)} placeholder="name@example.com" type="email" value={email} aria-invalid={email.length > 0 && !isEmail(email)} /><FieldDescription>کاربر باید پیش‌تر یک حساب فعال در فروشگاه ساخته باشد.</FieldDescription>{email.length > 0 && !isEmail(email) ? <FieldError>ایمیل معتبر نیست.</FieldError> : null}</Field><RoleChoices roles={availableRoles(roles, canAssignOwner)} selected={selected} onChange={setSelected} /><DialogFooter><Button disabled={disabled || !valid} type="submit">{disabled ? 'در حال افزودن…' : 'افزودن به تیم'}</Button><Button disabled={disabled} onClick={() => setOpen(false)} type="button" variant="outline">انصراف</Button></DialogFooter></form></DialogContent></Dialog></>;
}
...
function RoleChoices({ roles, selected, onChange }: { readonly roles: readonly StaffRole[]; readonly selected: readonly string[]; readonly onChange: (keys: readonly string[]) => void }) {
  ...
  return <FieldSet><legend className="text-sm font-medium">نقش‌ها</legend><div className="grid gap-2 sm:grid-cols-2">{roles.map((role) => { const checked = selected.includes(role.key); const presentation = rolePresentation(role.key); return <label className="flex cursor-pointer items-start gap-3 rounded-lg border p-3 transition-colors has-data-[state=checked]:border-primary has-data-[state=checked]:bg-primary/5" key={role.key}><Checkbox checked={checked} onCheckedChange={() => onChange(checked ? selected.filter((key) => key !== role.key) : [...selected, role.key])} /><span className="min-w-0"><span className="block text-sm font-medium">{presentation.label}</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">{presentation.description}</span></span></label>; })}</div></FieldSet>;
}
...
function Loading() { return <section aria-busy="true" className="mx-auto max-w-6xl space-y-5" dir="rtl"><Skeleton className="h-9 w-52" /><div className="grid gap-4 xl:grid-cols-2">{Array.from({ length: 4 }, (_, index) => <Skeleton className="h-48" key={index} />)}</div></section>; }
function Failure({ error, onRetry }: { readonly error: unknown; readonly onRetry: () => void }) { return <section className="mx-auto max-w-xl rounded-xl border bg-card p-6" dir="rtl" role="alert"><ShieldCheck className="size-8 text-destructive" /><h1 className="mt-4 font-semibold">فهرست کارکنان دریافت نشد</h1><p className="mt-2 text-sm leading-6 text-muted-foreground">{message(error)}</p><Button className="mt-4" onClick={onRetry} variant="outline"><RefreshCw aria-hidden="true" /> تلاش دوباره</Button></section>; }
...
const rolePresentations: Record<string, { label: string; description: string }> = { owner: { label: 'مالک', description: 'دسترسی کامل، شامل مدیریت مالکان' }, administrator: { label: 'مدیر', description: 'مدیریت عملیات فروشگاه بدون اختیار تعیین مالک' }, catalog_manager: { label: 'مدیر کاتالوگ', description: 'مدیریت کالاها، قیمت و موجودی' }, order_manager: { label: 'مدیر سفارش‌ها', description: 'رسیدگی به سفارش، پرداخت و ارسال' }, support_agent: { label: 'پشتیبانی', description: 'مشاهده مشتریان و ثبت یادداشت سفارش' }, marketing_manager: { label: 'مدیر بازاریابی', description: 'مدیریت ترویج و گزارش‌های بازاریابی' }, analyst: { label: 'تحلیل‌گر', description: 'مشاهده گزارش‌ها بدون امکان ویرایش' } };
function rolePresentation(key: string) { return rolePresentations[key] ?? { label: key, description: 'نقش سفارشی فروشگاه' }; }
function roleLabel(key: string) { return rolePresentation(key).label; }
